...
class ToneResult {
  ...
  factory ToneResult.fromJson(Map<String, dynamic> j) => ToneResult(
        ...
        std: (j['std'] as num?)?.toDouble() ?? 0,
        minVal: j['minVal'] as int? ?? 0,
        maxVal: j['maxVal'] as int? ?? 0,
        peakPosition: (j['peakPosition'] as num?)?.toDouble() ?? 0,
        shadows: (j['shadows'] as num?)?.toDouble() ?? 0,
        midtones: (j['midtones'] as num?)?.toDouble() ?? 0,
        highlights: (j['highlights'] as num?)?.toDouble() ?? 0,
        toneKey: j['toneKey'] as String? ?? 'mid',
        toneRange: j['toneRange'] as String? ?? 'medium',
  ...
  static ToneResult? fromJsonString(String? json) {
    ...
    try {
      ...
      debugPrint('ToneResult.fromJsonString failed: $e');
      return null;
    }
  }
}
