...
class _DetailPageState extends ConsumerState<DetailPage> {
  ...
  void _handleColorPickUpdate(LongPressMoveUpdateDetails details) {
    ...
    // 节流：至少 80ms 间隔，避免每帧全量解码大图导致 OOM/卡顿
    _pickColorThrottle?.cancel();
    _pickColorThrottle = Timer(const Duration(milliseconds: 80), () {
      _pickColorAt(details.globalPosition);
    });
  }
  ...
  void _handleColorPickEnd(LongPressEndDetails details) {
    ...
    // fire-and-forget：DB 写入失败仅记录日志，不影响 UI
    unawaited(
      ref.read(appDatabaseProvider).colorPinDao.insertPin(pin).then((_) {
        // success
      }).catchError((Object e) {
        debugPrint('Failed to save color pin: $e');
      }),
    );
