            ...
            return Column(
              children: [
                ...
                Expanded(
                  ...
                  child: Row(
                    children: [
                      Expanded(child: _buildImage(context, p1.filePath)),
                      Container(width: 1, color: Colors.white12),
                      Expanded(child: _buildImage(context, p2.filePath)),
                    ],
                  ),
                ),
                // 对比分析面板
    ...
    return InteractiveViewer(
      ...
      child: Center(
        child: Image.file(
          ...
          fit: BoxFit.contain,
          cacheWidth:
              (MediaQuery.sizeOf(context).width * 1.5).toInt().clamp(1, 2048),
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: TabBarView(
              children: [
                // 直方图对比（懒加载：histogram 只在此 widget 内 watch）
                _HistogramCompare(
                  photoId1: photoId1,
                  photoId2: photoId2,
                ),
...
/// 直方图对比（亮度叠加）
class _HistogramCompare extends ConsumerWidget {
  final String photoId1;
  final String photoId2;

  const _HistogramCompare({required this.photoId1, required this.photoId2});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return hist1.when(
      ...
      data: (h1) => hist2.when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (_, __) => const Center(child: Text('加载失败')),
        data: (h2) => _HistogramCompareContent(data1: h1, data2: h2),
      ),
    );
  }
}

/// 直方图对比内容（数据已就绪后的纯展示组件）
class _HistogramCompareContent extends StatelessWidget {
  final HistogramData data1;
  final HistogramData data2;

  const _HistogramCompareContent({required this.data1, required this.data2});
      ...
      data: (tone) {
        ...
        return Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _zoneBar(context, '暗部', t.shadows),
            const SizedBox(height: 4),
            _zoneBar(context, '中间调', t.midtones),
            const SizedBox(height: 4),
            _zoneBar(context, '亮部', t.highlights),
            const SizedBox(height: 8),
