...
class _AlbumDetailPageState extends ConsumerState<AlbumDetailPage> {
  ...
  Future<void> _addPhotos() async {
    ...
    final importResult = await importService.importPhotos(filePaths);

    // 将新导入的照片加入相册（用 importResult.importedPhotoIds 精确获取）
    final db = ref.read(appDatabaseProvider);
    for (final photoId in importResult.importedPhotoIds) {
      await db.albumDao.addPhotoToAlbum(widget.albumId, photoId);
    }

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('已添加 ${importResult.importedPhotoIds.length} 张照片到相册')),
      );
    }
  }
