...
final histogramProvider =
  ...
  // 缓存到数据库（Uint16List 格式，hue 独立列）— 写入失败不影响返回结果
  try {
    ...
    debugPrint('histogram cache write failed: $e');
  }
  return hist;
});
...
final paletteProvider =
  ...
  if (isDefault) {
    try {
      ...
      debugPrint('palette cache write failed: $e');
    }
  }
  return palette;
});
...
final toneProvider =
  ...
  try {
    ...
    debugPrint('tone cache write failed: $e');
  }
  return tone;
});
